...
const addOrder = async (req, res) => {
    ...
    try {
        ...
        if (!freezoneItemId)
            return;
        const distributionItem = await _services_1.distributionServices.addDistributionItem(req, res, freezoneItemId);
        return (0, _helpers_1.sendResponse)(res, 201, `შეკვეთა წარმატებით დაემატა; შეკვეთა წარმატებით დაემატა თავისუფალ ზონაში; შეკვეთა წარმატებით დაემატა დისტრიბუციაში`, distributionItem);
    }
    catch (error) {
...
const getOrder = async (req, res) => {
    ...
    try {
        ...
        if (!order.exists)
            return (0, _helpers_1.sendResponse)(res, 404, 'შეკვეთა ვერ მოიძებნა', order.order);
...
const getOrders = async (req, res) => {
    try {
        ...
        if (!foundOrders.exists)
            return (0, _helpers_1.sendResponse)(res, 404, 'შეკვეთები ვერ მოიძებნა', foundOrders.orders);
...
const updateOrder = async (req, res) => {
    const data = req.body;
    const parsedData = _validations_1.orderSchema.safeParse(data);
    if (!parsedData.success)
        return (0, _helpers_1.sendResponse)(res, 400, 'Validation error', parsedData.error.format());
    try {
        ...
        return (0, _helpers_1.sendResponse)(res, 200, 'შეკვეთა წარმატებით განახლდა', updatedOrder.order);
    }
    catch (error) {
