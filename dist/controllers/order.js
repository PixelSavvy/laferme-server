...
const addOrder = async (req, res) => {
  ...
  try {
    const orderId = await _services_1.orderServices.addOrder(
      req,
      res,
      parsedData.data,
    );
    if (!orderId) {
      ...
      return (0, _helpers_1.sendResponse)(
        res,
        500,
        "შეცდომა შეკვეთის შექმნისას",
      );
    }
    const freezoneItemId = await _services_1.freezoneServices.addFreezoneItem(
      req,
      res,
      orderId,
    );
...
const getOrder = async (req, res) => {
  const { id } = req.params;
  try {
    const order = await _services_1.orderServices.getOrder(
      req,
      res,
      Number(id),
    );
...
const updateOrder = async (req, res) => {
  ...
  try {
    const updatedOrder = await _services_1.orderServices.updateOrder(
      req,
      res,
      parsedData.data,
    );
    ...
    const updatedFreezoneItem =
      await _services_1.freezoneServices.updateFreezoneItemOnOrderUpdate(
        req,
        res,
        parsedData.data,
      );
