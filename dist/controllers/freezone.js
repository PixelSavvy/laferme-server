...
const getFreezoneItem = async (req, res) => {
    const { id } = req.params;
    try {
        const freezoneItem = await _services_1.freezoneServices.getFreezoneItem(req, res, Number(id));
        return (0, _helpers_1.sendResponse)(res, 200, 'შეკვეთა თავისუფალ ზონაში წარმატებით მოიძებნა!', freezoneItem);
    }
    catch (error) {
...
const getFreezoneItems = async (req, res) => {
    const { id } = req.params;
    try {
        const freezoneItems = await _services_1.freezoneServices.getFreezoneItems(req, res, Number(id));
        if (!freezoneItems.exists)
            return (0, _helpers_1.sendResponse)(res, 404, 'შეკვეთები თავისუფალ ზონაში ვერ მოიძებნა', freezoneItems.freezoneItems);
        return (0, _helpers_1.sendResponse)(res, 200, 'შეკვეთები თავისუფალ ზონაში წარმატებით მოიძებნა!', freezoneItems.freezoneItems);
    }
    catch (error) {
        console.error('Error fetching an order', error);
...
const updateFreezoneItem = async (req, res) => {
    ...
    try {
        ...
        if (!updatedOrder.exists)
            return (0, _helpers_1.sendResponse)(res, 404, 'შეკვეთა ვერ მოიძებნა', updatedOrder.freezoneItem);
