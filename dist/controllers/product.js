...
const getProducts = async (req, res) => {
  try {
    const foundProducts = await _services_1.productServices.getProducts(
      req,
      res,
    );
...
const getProduct = async (req, res) => {
  ...
  console.log(id);
  try {
    const foundProduct = await _services_1.productServices.getProduct(
      req,
      res,
      id,
    );
...
const addProduct = async (req, res) => {
  ...
  try {
    // Add a new product
    const result = await _services_1.productServices.addProduct(
      req,
      res,
      parsedData.data,
    );
...
const updateProduct = async (req, res) => {
  ...
  try {
    const updatedProduct = await _services_1.productServices.updateProduct(
      req,
      res,
      parsedData.data,
    );
    ...
    return (0, _helpers_1.sendResponse)(
      res,
      505,
      "შეცდომა პროდუქტის რედაქტირებისას",
      error,
    );
  }
};
