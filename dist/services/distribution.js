...
const distributionItem_1 = require("@/models/distributionItem");
const distributionItemProduct_1 = require("@/models/distributionItemProduct");
const sequelize_1 = require("sequelize");
const addDistributionItem = async (req, res, freezoneItemId) => {
    const transaction = await lib_1.sequelize.transaction();
    try {
        const newDistributionItem = await distributionItem_1.DistributionItem.create({
            freezoneItemId,
            status: config_1.distributionStatus[3000],
        }, { transaction });
        // Fetch associated freezone products
        const freezoneItemProducts = await models_1.FreezoneItemProduct.findAll({
            where: {
                freezoneItemId,
            },
            raw: true,
        ...
        const distributionItemProducts = await distributionItemProduct_1.DistributionItemProduct.bulkCreate(transformedDistributionItemProducts, {
            transaction,
        });
...
const getDistributionItem = async (req, res, id) => {
    try {
        const distributionItem = await distributionItem_1.DistributionItem.findByPk(id, {
            attributes: {
                exclude: ['freezoneItemId'],
            },
            include: [
                {
                    model: models_1.FreezoneItem,
                    as: 'freezone',
                },
                {
        ...
        if (!distributionItem)
            return (0, helpers_1.sendResponse)(res, 404, 'მსგავსი დისტრიბუცია ვერ მოიძებნა');
...
const getDistributionItems = async (req, res) => {
    try {
        const existingDistributionItems = await distributionItem_1.DistributionItem.findAll({
            include: [
                {
                    model: models_1.FreezoneItem,
                    as: 'freezone',
...
const updateDistributionItem = async (req, res, data) => {
    const transaction = await lib_1.sequelize.transaction();
    try {
        const existingDistributionItem = await distributionItem_1.DistributionItem.findByPk(data.id, { transaction });
        ...
        await distributionItemProduct_1.DistributionItemProduct.destroy({
            where: { distributionItemId: updatedDistributionItem.id },
            transaction,
        });
        await distributionItemProduct_1.DistributionItemProduct.bulkCreate(distributionItemProducts, { transaction });
