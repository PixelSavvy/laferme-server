...
const addOrder = async (req, res, data) => {
  ...
  try {
    const existingCustomer = await _models_1.Customer.findByPk(
      data.customerId,
      { transaction },
    );
    if (!existingCustomer) {
...
const updateOrder = async (req, res, data) => {
  ...
  try {
    ...
    const existingOrder = await _models_1.Order.findByPk(data.id, {
      transaction,
    });
    ...
    if (products && Array.isArray(products)) {
      ...
      await _models_1.OrderProduct.bulkCreate(orderProductsToAdd, {
        transaction,
      });
