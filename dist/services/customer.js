...
const addCustomer = async (req, res, data) => {
    ...
    try {
        ...
        const existingCustomer = await models_1.Customer.findOne({
            where: {
                email: data.email,
            },
            transaction,
        ...
        if (existingCustomer) {
            ...
            return (0, helpers_1.sendResponse)(res, 409, 'მომხმარებელი მსგავსი ელ.ფოსტით უკვე არსებობს');
        }
        ...
        const newCustomer = await models_1.Customer.create({
            ...data,
            priceIndex: config_1.priceIndex[Number(data.priceIndex)],
            paymentOption: config_1.paymentOptions[Number(data.paymentOption)],
        });
...
const getCustomers = async (req, res) => {
    try {
        const foundCustomer = await models_1.Customer.findAll({
            include: [
                {
...
const updateCustomer = async (data) => {
    const transaction = await lib_1.sequelize.transaction();
    try {
        const existingCustomer = await models_1.Customer.findByPk(data.id, { transaction });
        if (!existingCustomer) {
            await transaction.rollback();
...
const deleteCustomer = async (req, res, id) => {
    const transaction = await lib_1.sequelize.transaction();
    try {
        const foundCustomer = await models_1.Customer.findByPk(id, { transaction });
        if (!foundCustomer) {
            await transaction.rollback();
            return (0, helpers_1.sendResponse)(res, 404, 'სარეალიზაციო პუნქტი მსგავსი საიდენტიფიკაციო კოდით ვერ მოიძებნა!');
        }
        await foundCustomer.destroy({ transaction });
        // Verify the customer is no longer in the database
        const checkDeleted = await models_1.Customer.findByPk(id, { transaction });
        if (checkDeleted) {
            await transaction.rollback();
            return (0, helpers_1.sendResponse)(res, 500, 'სარეალიზაციო პუნქტის წაშლა ვერ მოხერხდა!');
        }
        await transaction.commit();
        return (0, helpers_1.sendResponse)(res, 200, 'სარეალიზაციო პუნქტი წარმატებით წაიშალა!');
    }
    catch (error) {
        await transaction.rollback();
        throw error;
