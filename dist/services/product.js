...
const addProduct = async (req, res, data) => {
  ...
  try {
    const existingProduct = await _models_1.Product.findOne({
      where: {
    ...
    const newProduct = await _models_1.Product.create(data);
    return {
      exists: false,
      product: newProduct,
    };
...
const getProducts = async (req, res) => {
  try {
    const foundProduct = await _models_1.Product.findAll();
    return foundProduct;
  } catch (error) {
    throw error;
  }
...
const getProduct = async (req, res, id) => {
  try {
    const foundProduct = await _models_1.Product.findByPk(id);
    return foundProduct;
  } catch (error) {
    throw error;
  }
...
const updateProduct = async (req, res, data) => {
  const transaction = await _lib_1.sequelize.transaction();
  try {
    const existingProduct = await _models_1.Product.findByPk(data.id, {
      transaction,
    });
    if (!existingProduct)
      return {
